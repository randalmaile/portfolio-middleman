...
////////////////////Using jQuery-Tumblr Kit to pull Tumblr posts from API////////////////////
$(function() {
  var monthNames = [ "Jan", "Feb", "Mar", "Apr", "May", "Jun",
    "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" ];
  $("ul#tumblr-posts").getTumblrPosts({
    type: "link",
    limit: 5,
    done: function(data, textStatus, jqXHR, url) {
      $.each(data.response.posts,function(k,v) {
          var raw = v.date;
          var conv = raw.replace(/\s+/g, 'T');
          conv = conv.substring(0, conv.length - 4);
          var post_date = new Date(conv);
          var post_dateDay = post_date.getDate();
          var post_dateMonth = monthNames[post_date.getMonth()];
          $(".news span.day").eq(k).append(post_dateDay);
          $(".news span.month").eq(k).append(post_dateMonth);
      });
    }
  });
}); 
...
$('.gallery-item .gallery-show-modal').click(function (e, param) {
    ...
    // $modal.find('.close-button').click(function () {
      $modal.find('.icon-remove').click(function () {
      $modal.fadeOut({
        complete: function () {
          $modal.remove();
        }
    ...
    $('#overlay').click(function () {
      ...
      $modal.find('.icon-remove').trigger('click');
    });
